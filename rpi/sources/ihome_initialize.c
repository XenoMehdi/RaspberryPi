...
// local data
  ...
  if (signal(SIGINT, ihome_signal_int) == SIG_ERR)
    log_print("Cannot handle SIGINT!\n");
  if (signal(SIGHUP, ihome_signal_hup) == SIG_ERR)
    log_print("Cannot handle SIGHUP!\n");
...
// initialize config array
  ...
  context_1.next = &context_2 ;
  context_pile = &context_1 ;
